...
class HistoricoController extends AbstractController{
    ...
    protected initRoutes(): void {
        ...
        //Añadir un solo KPI a la base de datos hecho para pruebas
        this.router.post('/crearKPI',this.postCrearKPI.bind(this));
    ...
    private async postCrearKPI(req: Request,res: Response){
        try{
            console.log(req.body);
            await KPIPruebaModel.create(req.body);
            console.log("KPI creado");
            res.status(200).send("<h1>KPI creado</h1>");
        }catch(err){
            console.log(err);
    ...
    private async consultaAbandono(req: Request,res: Response){
        try{
            //Se define la información que se quiere buscar en la tabla de dynamoDB
            ...
            //Se recorre la información obtenida de la base de datos para obtener los datos necesarios en la estructura deseada
            if (metricas.Items) {
            ...
            //Se ordenan los datos obtenidos por fecha
    ...
    private async consultaAbandonoMn(req: Request,res: Response){
        try{
            //En base a la cantidad de minutos pedidos se obtiene la información de la base de datos
            const min:any = req.query.cantidad;

            const minC = parseInt(min);
